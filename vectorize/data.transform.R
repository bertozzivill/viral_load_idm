data.transform<-function(upper_bound=3.5,debias=1,impute_type='no_vars',impute.with.aids=T,imputation_count=2,should.we.impute=T){

source('vectorize/impute.R')
  
print("imputing")

if(debias==1){surv[,log_event_time:= log(event_timeNew)]}else{
  surv[,log_event_time:= log(event_time)]}

surv[, event_num:=ifelse(event_type=="mar", NA, ifelse(event_type=="aids", 1, 2))] #need to imput event type as well as time now
...
##cross-validation??
save(to_impute, file=paste0("vectorize/","impute_data.rdata"))


#run and save imputations
if (should.we.impute){
  impute(upper_bound, imputation_count)
}else{
  load("../data/bounded_imputations.rdata")
  all_imputations <- copy(all_bounded)
}
...
load(file=paste0("vectorize/", "imputed_data.rdata"))
